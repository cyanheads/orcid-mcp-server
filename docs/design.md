...
| `orcid://researcher/{orcid_id}/works` | Works list for a researcher. Use when providing a researcher's publication list as background context, e.g., before asking an LLM to summarize a body of work. DOIs and PMIDs in the response are ready for Crossref/PubMed chaining. | Stable inline context; prefer the tool when filtering or processing results is needed. | No |
